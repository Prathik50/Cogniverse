...
const CommunicationBoardScreen = ({ onBack }) => {
  ...
  const getInitialSymbols = () => [
    { id: '1', label: t('symbols.i'), type: 'word' },
    { id: '2', label: t('symbols.want'), type: 'word' },
    { id: '3', label: t('symbols.play'), type: 'word' },
    { id: '4', label: t('symbols.eat'), type: 'word' },
    { id: '5', label: t('symbols.help'), type: 'word' },
    { id: '6', label: t('symbols.happy'), type: 'word' },
  ];
  ...
  const handleGenerateSymbol = async () => {
    ...
    try {
      ...
      if (result.predictions && result.predictions[0] && result.predictions[0].bytesBase64Encoded) {
        ...
        speak(`${t('createdSymbol')} ${customWord}`);
      } else {
        Alert.alert(t('error'), t('errorGeneratingImage'));
        console.error('API Error:', result);
      }
    } catch (error) {
      Alert.alert(t('error'), t('errorConnectingAI'));
      ...
      <View style={dynamicStyles.inputContainer}>
        ...
        <TouchableOpacity style={dynamicStyles.generateButton} onPress={handleGenerateSymbol} disabled={isLoading}>
          {isLoading ? (
            ...
            <Text style={dynamicStyles.generateButtonText}>{t('create')}</Text>
          )}
        </TouchableOpacity>
      </View>
